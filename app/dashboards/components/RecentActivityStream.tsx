  ...
  return (
    <section className="flex max-h-[400px] flex-col overflow-hidden rounded-[13px] border border-border/80 bg-surface shadow-[var(--shadow-float)]">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {auditLogs.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center rounded-xl border border-dashed border-border/80 bg-surface-hover/35 px-6 text-center shadow-sm">
          ...
          <div className="space-y-2.5">
            {auditLogs.map((act) => {
              ...
              return (
                <div
                  key={act.id}
                  className="flex items-start gap-3 rounded-xl border border-border/80 bg-background px-3 py-2.5 transition-colors duration-150 hover:bg-surface-hover shadow-sm hover:shadow-md"
                >
