                ...
                <tr key={run.id} className="border-b border-slate-100 hover:bg-slate-50/80 transition-colors group">
                  <td className="px-6 py-4">
                    <Link href={`/projects/${run.project.code}/runs/${run.id}`} className="flex flex-col">
                      ...
                      <div className="flex items-center mt-1.5">
                        <span className="text-[10px] font-extrabold text-indigo-600 bg-indigo-50 border border-indigo-100/50 px-1.5 py-0.5 rounded shadow-sm">{run.project.code}-{run.id.split('-')[0]}</span>
                  ...
                  <td className="px-6 py-4">
                     ...
                     {run.liveStatus === 'MANUAL_ACTIVE' && (
                       <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-sky-50 text-sky-700 border border-sky-200/60 shadow-sm">
                         <PlayCircle size={12} className="mr-1.5 text-sky-500" /> Active (Manual)
                       </span>
                     )}
                  ...
                  <td className="px-6 py-4">
                    <div className={`w-full h-1.5 bg-slate-100 rounded-full flex overflow-hidden mb-1.5 ${run.liveStatus === 'RUNNING' ? 'opacity-100' : 'opacity-80'}`}>
                      ...
                      <div style={{ width: `${untestedPercent}%` }} className="bg-slate-200/60" title={`Untested: ${run.metrics.untested}`} />
                    </div>
                    <div className="flex space-x-2 text-[11px] font-bold mt-1.5">
                      {run.metrics.passed > 0 && <span className="text-emerald-600 bg-emerald-50 px-1.5 rounded">{run.metrics.passed}P</span>}
                      {run.metrics.failed > 0 && <span className="text-red-600 bg-red-50 px-1.5 rounded">{run.metrics.failed}F</span>}
                      {run.metrics.untested > 0 && <span className="text-slate-500 bg-slate-100 px-1.5 rounded">{run.metrics.untested}U</span>}
                    </div>
                  </td>
                </tr>
              )
