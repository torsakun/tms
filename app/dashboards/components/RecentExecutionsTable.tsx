...
import { PlayCircle, ChevronRight, ChevronLeft, Clock } from "lucide-react";
...
export function RecentExecutionsTable({ recentRuns }: RecentExecutionsTableProps) {
  ...
  const currentRuns = recentRuns.slice(startIndex, startIndex + itemsPerPage);
      ...
      {totalPages > 1 && (
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 mt-auto">
          <span className="text-xs text-slate-500 font-medium">
            Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, recentRuns.length)} of {recentRuns.length}
          </span>
