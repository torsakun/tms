...
function CreateCaseContent({ params }: { params: any /* use React.use() if async in Next15, but it's client comp here */ }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="h-full overflow-y-auto w-full bg-slate-50 pb-20">
      {/* Sticky Header */}
      <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-xl font-semibold text-slate-800">Create test case</h1>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="flex items-center text-sm font-semibold text-slate-700 mb-2">
                <Folder size={14} className="mr-1.5 text-slate-400" /> Suite
              </label>
              <select
                {...register("suiteId", { required: "Suite is required" })}
                className={cn(
                  "w-full px-3 py-2.5 bg-slate-50 border rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none appearance-none cursor-pointer transition-all",
                  errors.suiteId ? "border-red-500" : "border-slate-200"
                )}
              >
                ...
                {attachments.map((file, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 bg-slate-50 border border-slate-200 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-xs font-medium text-slate-700 truncate max-w-[200px]">{file.originalName}</p>
