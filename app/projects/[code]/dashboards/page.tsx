...
export default async function ProjectOverviewPage({
  ...
  const severityConfig: Record<
    string,
    { color: string; bar: string; label: string }
  > = {
    ...
    TRIVIAL: { color: "text-text-muted", bar: "bg-slate-400", label: "Trivial" },
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-text-muted">
              {metrics.totalCases} test cases · {metrics.totalSuites} suites ·{" "}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div
            ...
            style={{
              ...
              boxShadow: "0 4px 20px rgba(8,145,178,0.30)",
            }}
          >
            <div className="absolute -right-4 -bottom-4 w-24 h-24 rounded-full bg-surface/10" />
            <div className="flex items-center justify-between mb-3">
              ...
              {recentRuns.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-text-muted text-xs">
                    Start a test run to track execution results
                  </p>
                  <Link
                  ...
                  {recentRuns.map((run) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center gap-4 p-4 rounded-xl border border-border hover:border-indigo-200 hover:bg-indigo-50/30 transition-all group"
                      >
                        <div
                          className={`w-2 h-10 rounded-full shrink-0 ${isActive ? "bg-indigo-400 animate-pulse" : allPassed ? "bg-emerald-400" : hasFailed ? "bg-red-400" : "bg-slate-300"}`}
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-2">
                            <span className="font-semibold text-text-main text-sm truncate group-hover:text-indigo-600 transition-colors">
                              {run.title}
                            </span>
                            <span className="text-[11px] text-text-muted shrink-0 ml-3">
                          ...
                          <div className="w-full h-1.5 bg-surface-hover rounded-full flex overflow-hidden">
                            <div
                              style={{ width: `${passP}%` }}
          ...
          <div className="bg-amber-50 rounded-2xl border border-amber-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {outdatedCases.map((tc) => (
                <div
                  key={tc.id}
                  className="bg-surface border border-amber-200 p-4 rounded-xl shadow-sm"
                >
                  <div className="flex items-center justify-between mb-2">
