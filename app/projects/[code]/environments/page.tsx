      ...
      <main className="flex-1 overflow-y-auto p-8">
        {environments.length === 0 ? (
          <div className="text-center py-20 bg-surface rounded-lg border border-border border-dashed">
            <Box size={48} className="mx-auto text-text-muted opacity-50 mb-4" />
            <h3 className="text-lg font-medium text-text-main mb-2">No environments yet</h3>
            <p className="text-text-muted max-w-sm mx-auto mb-6 text-sm">
              Create test environments (like Staging, Production, iOS, Android) to specify where your tests are executed.
            </p>
            <button 
              onClick={() => setIsModalOpen(true)}
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border">
                {environments.map(env => (
                  <tr key={env.id} className="hover:bg-surface-hover transition-colors">
                    <td className="px-6 py-4 font-medium text-text-main">{env.title}</td>
                    <td className="px-6 py-4 text-text-muted font-mono text-xs">{env.slug}</td>
                    <td className="px-6 py-4 text-text-muted truncate max-w-xs">{env.description || "-"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface w-[500px] rounded-lg shadow-xl overflow-hidden border border-border animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-background">
              <h3 className="text-lg font-bold text-text-main">Create environment</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-text-muted hover:text-text-main transition-colors">
                <Settings size={20} className="hidden" /> {/* just placeholder */}
                &times;
              </button>
            </div>
