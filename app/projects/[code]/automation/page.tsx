    ...
    <div className="flex h-[calc(100vh-4rem)] bg-surface-hover dark:bg-[#0B0F19] overflow-y-auto">
      <div className="w-full max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-text-main dark:text-text-main flex items-center tracking-tight">
              TESSA <span className="mx-3 text-slate-300 dark:text-text-main font-light">|</span> 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-surface dark:bg-surface rounded-2xl p-6 border border-border dark:border-border shadow-sm backdrop-blur-sm relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <p className="text-sm font-medium text-text-muted dark:text-text-muted mb-1">AI Generated Scripts</p>
                <h3 className="text-3xl font-bold text-text-main dark:text-text-main">{stats.automated}</h3>
              </div>
          ...
          <div className="bg-surface dark:bg-surface rounded-2xl p-6 border border-border dark:border-border shadow-sm backdrop-blur-sm relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <p className="text-sm font-medium text-text-muted dark:text-text-muted mb-1">Executive ROI</p>
                <h3 className="text-3xl font-bold text-text-main dark:text-text-main">${analytics?.roi?.estimatedValueUsd || "0"}</h3>
              </div>
          ...
          <div className="bg-surface dark:bg-surface rounded-2xl p-6 border border-border dark:border-border shadow-sm backdrop-blur-sm relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <p className="text-sm font-medium text-text-muted dark:text-text-muted mb-1">Automation Coverage</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-surface dark:bg-[#111623] rounded-2xl border border-border dark:border-border overflow-hidden flex flex-col shadow-sm h-[500px]">
            <div className="px-6 py-4 border-b border-border dark:border-border flex justify-between items-center bg-surface-hover/50 dark:bg-surface">
              ...
              <span className="text-xs font-medium text-text-muted bg-slate-200 dark:bg-slate-800 px-2 py-1 rounded-md">{selectedCaseIds.length} Selected</span>
            </div>
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="overflow-y-auto flex-1">
                <table className="w-full text-left text-sm text-text-muted dark:text-slate-300">
                  <thead className="bg-surface-hover dark:bg-surface border-b border-border dark:border-border text-xs uppercase font-semibold text-text-muted dark:text-text-muted sticky top-0">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 bg-surface dark:bg-[#111623]">
                    {cases.length === 0 ? (
                      <tr>
                      ...
                      return (
                        <tr key={tc.id} className="hover:bg-surface-hover dark:hover:bg-slate-900/50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {isManual ? (
                               ...
                               <div className="flex items-center space-x-2">
                                 ...
                                 <button onClick={() => handleOpenReview(tc)} className="text-text-muted hover:text-indigo-500 transition-colors bg-surface-hover hover:bg-indigo-50 dark:bg-slate-800 dark:hover:bg-indigo-500/20 p-1 rounded" title="View/Edit Code">
                                   <Eye size={14} />
                                 </button>
                                 <button onClick={() => handleDiscard(tc.id)} className="text-text-muted hover:text-red-500 transition-colors bg-surface-hover hover:bg-red-50 dark:bg-slate-800 dark:hover:bg-red-500/20 p-1 rounded" title="Discard">
                                   <Trash2 size={14} />
                                 </button>
                               </div>
                            ) : null}
          ...
          <div className="space-y-8 flex flex-col">
            ...
            {/* Flakiness Radar */}
            <div className="bg-surface dark:bg-[#111623] rounded-2xl border border-border dark:border-border shadow-sm flex-1">
              <div className="px-6 py-4 border-b border-border dark:border-border flex justify-between items-center bg-surface-hover/50 dark:bg-surface rounded-t-2xl">
                ...
                <span className="text-xs bg-surface-hover dark:bg-slate-800 text-text-muted px-2 py-1 rounded-full font-medium">Top {analytics?.flakiness?.length || 0}</span>
              </div>
              <div className="p-0">
                {(!analytics?.flakiness || analytics.flakiness.length === 0) ? (
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                    {analytics.flakiness.map((item: any) => (
                      <div key={item.caseId} className="p-4 hover:bg-surface-hover dark:hover:bg-slate-900/50 transition-colors flex items-center justify-between group">
                        <div className="flex-1 min-w-0 mr-4">
                          <h4 className="text-sm font-semibold text-text-main dark:text-slate-200 truncate">{item.title}</h4>
                          <div className="flex items-center mt-1.5 space-x-1">
                             ...
                             <span className="text-[10px] text-text-muted ml-2 font-medium">Score: {item.flakinessScore}%</span>
                          </div>
                        </div>
            ...
            {/* Pipeline Status */}
            <div className="bg-surface dark:bg-[#111623] rounded-2xl border border-border dark:border-border shadow-sm">
              <div className="px-6 py-4 border-b border-border dark:border-border flex justify-between items-center bg-surface-hover/50 dark:bg-surface rounded-t-2xl">
                <h3 className="font-semibold text-text-main dark:text-text-main flex items-center">
                  <PlayCircle size={18} className="mr-2 text-primary" /> Pipeline Orchestration
                </h3>
              </div>
              <div className="p-6 space-y-4">
                {pipelines.length === 0 ? (
                  ...
                  <div key={pipeline.id} className="bg-surface-hover dark:bg-surface rounded-xl p-4 border border-border dark:border-border flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="font-semibold text-text-main dark:text-text-main text-sm">{pipeline.title}</h4>
                        <p className="text-xs text-text-muted dark:text-text-muted">Schedule: <code className="bg-slate-200 dark:bg-slate-800 px-1 rounded">{pipeline.cron}</code></p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
                      <button 
                        onClick={() => handleTriggerPipeline(pipeline.id)}
                        className="text-xs font-bold px-3 py-1.5 bg-blue-500/10 text-primary dark:text-blue-400 hover:bg-blue-500/20 rounded-lg transition-colors flex items-center"
                      >
                      ...
                      <label className="flex items-center cursor-pointer group">
                        <div className="relative">
                          ...
                          <div className={`absolute left-[2px] top-[2px] bg-surface w-5 h-5 rounded-full transition-transform transform ${pipeline.isActive ? 'translate-x-4' : ''} shadow-sm`}></div>
                        </div>
                      ...
                      <button 
                        ...
                        className="text-xs font-bold px-2 py-1.5 text-text-muted hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors flex items-center"
      ...
      {reviewCase && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface dark:bg-surface w-full max-w-4xl rounded-2xl shadow-2xl border border-border dark:border-border overflow-hidden flex flex-col h-[80vh]">
            <div className="px-6 py-4 border-b border-border dark:border-border flex justify-between items-center bg-surface-hover dark:bg-surface/80">
              ...
              <button onClick={() => setReviewCase(null)} className="text-text-muted hover:text-text-muted dark:hover:text-slate-200">
                <X size={20} />
              </button>
            </div>
            
            <div className="p-4 bg-surface-hover dark:bg-slate-950/50 border-b border-border dark:border-border text-sm">
              <span className="font-semibold text-text-main dark:text-slate-300">Test Case:</span> {reviewCase.title}
            </div>
            ...
            <div className="px-6 py-4 border-t border-border dark:border-border bg-surface-hover dark:bg-surface/80 flex justify-end space-x-3">
              <button 
                ...
                className="px-4 py-2 text-text-muted dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 font-medium rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button 
      ...
      {isPipelineModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface dark:bg-[#111623] w-full max-w-md rounded-2xl shadow-2xl border border-border dark:border-border overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-border dark:border-border flex justify-between items-center bg-surface-hover dark:bg-surface">
              ...
              <button onClick={() => setIsPipelineModalOpen(false)} className="text-text-muted hover:text-text-muted dark:hover:text-slate-200">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-text-main dark:text-slate-300 mb-1">Pipeline Title</label>
                <input 
                  type="text" 
                  value={newPipelineTitle}
                  onChange={(e) => setNewPipelineTitle(e.target.value)}
                  placeholder="e.g. Nightly Regression"
                  className="w-full bg-surface dark:bg-[#0d1117] border border-text-muted dark:border-slate-700 rounded-lg px-4 py-2 text-sm text-text-main dark:text-text-main focus:ring-2 focus:ring-indigo-500 outline-none"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-main dark:text-slate-300 mb-1">Schedule Frequency</label>
                <select
                  value={scheduleType}
                  onChange={(e) => setScheduleType(e.target.value)}
                  className="w-full bg-surface dark:bg-[#0d1117] border border-text-muted dark:border-slate-700 rounded-lg px-4 py-2 text-sm text-text-main dark:text-text-main focus:ring-2 focus:ring-indigo-500 outline-none mb-3"
                >
                ...
                {scheduleType !== 'custom' && (
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="block text-xs font-medium text-text-muted dark:text-text-muted mb-1">Time (UTC)</label>
                      <input 
                        type="time" 
                        value={scheduleTime}
                    ...
                    {scheduleType === 'weekly' && (
                      <div className="flex-1">
                        <label className="block text-xs font-medium text-text-muted dark:text-text-muted mb-1">Day of Week</label>
                        <select
                          value={scheduleDay}
                    ...
                    {scheduleType === 'monthly' && (
                      <div className="flex-1">
                        <label className="block text-xs font-medium text-text-muted dark:text-text-muted mb-1">Day of Month</label>
                        <input 
                          type="number" 
                          min="1" max="31"
                          value={scheduleDay}
                ...
                {scheduleType === 'custom' && (
                  <div>
                    <input 
                      ...
                      placeholder="0 0 * * *"
                      className="w-full bg-surface dark:bg-[#0d1117] border border-text-muted dark:border-slate-700 rounded-lg px-4 py-2 text-sm text-text-main dark:text-text-main font-mono focus:ring-2 focus:ring-indigo-500 outline-none"
                    />
                    <p className="text-xs text-text-muted mt-2">Example: <code>0 0 * * *</code> (Runs every midnight UTC).</p>
                  </div>
                )}
                
                {scheduleType !== 'custom' && (
                  <p className="text-xs text-text-muted mt-3 font-mono bg-surface-hover dark:bg-slate-800 p-2 rounded">Generated Cron: {generateCron()}</p>
                )}
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-border dark:border-border bg-surface-hover dark:bg-surface flex justify-end space-x-3">
              <button 
                ...
                className="px-4 py-2 text-text-muted dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 font-medium rounded-lg transition-colors text-sm"
              >
                Cancel
              </button>
              <button 
