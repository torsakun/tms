    ...
    <div className="max-w-6xl mx-auto">
      ...
      {plans.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-surface rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.03)] dark:shadow-none transition-colors">
          ...
          <h3 className="text-lg font-bold text-text-main mb-2">No Test Plans found</h3>
          <p className="text-text-muted text-center max-w-sm mb-6 text-sm">
            Create a test plan to group test cases together. You can use test plans to create runs faster.
          </p>
          <Link 
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-surface rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.03)] dark:shadow-none transition-colors">
          <p className="text-text-muted mb-2">No test plans match your search.</p>
          <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border/50 bg-surface-hover">
                <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider w-1/3">Title</th>
                <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider text-center">Cases</th>
                <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider text-center">Runs</th>
                <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider">Created</th>
                <th className="px-6 py-4 text-xs font-semibold text-text-muted uppercase tracking-wider text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {filteredPlans.map((plan) => (
                <tr key={plan.id} className="hover:bg-surface-hover transition-colors group">
                  <td className="px-6 py-4">
                    <Link href={`/projects/${code}/plans/${plan.id}`} className="font-semibold text-[15px] text-text-main hover:text-primary block transition-colors">
                      {plan.title}
                    </Link>
                    {plan.description && (
                      <div className="text-sm text-text-muted mt-1 line-clamp-1">{plan.description}</div>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <div className="relative" ref={activeDropdown === plan.id ? dropdownRef : null}>
                        <button 
