...
export async function POST(req: Request, { params }: { params: Promise<{ code: string; id: string }> }) {
  ...
  try {
    ...
    if (testCases.length === 0) {
      return NextResponse.json({ error: "No automated test cases found" }, { status: 400 });
    }
    ...
    if (project.githubOwner && project.githubRepo && project.githubToken && project.githubWorkflowId) {
      const dispatchUrl = `https://api.github.com/repos/${project.githubOwner}/${project.githubRepo}/actions/workflows/${project.githubWorkflowId}/dispatches`;
      ...
      if (!ghRes.ok) {
        console.error("Failed to trigger GitHub Action from Pipeline", await ghRes.text());
        return NextResponse.json({ error: "Created run but failed to trigger GitHub Action" }, { status: 500 });
      }
    } else {
      return NextResponse.json({ error: "GitHub integration not fully configured" }, { status: 400 });
    }
