...
export async function GET(req: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    const roi = {
      ...
      estimatedValueUsd: Math.round(totalHoursSaved * HOURLY_RATE_USD).toLocaleString(),
      estimatedValueThb: Math.round(totalHoursSaved * HOURLY_RATE_THB).toLocaleString()
    };
    ...
    for (const tc of allAutomatedCases) {
      ...
      if (results.length >= 2) {
        ...
        const flakinessScore = Math.round((transitions / (history.length - 1)) * 100);
