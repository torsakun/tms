      ...
      while (attempt < maxAttempts && !success) {
        ...
        const dom = await page.evaluate(() => {
          const elements = document.querySelectorAll(
            'button, a, input, select, textarea, [role="button"], [role="link"], [role="textbox"], [role="menuitem"]',
          );
          ...
          return visibleElements
            .map((el) => {
              ...
              if (e.getAttribute("type"))
                props.push(`type=${e.getAttribute("type")}`);
              if (e.getAttribute("name"))
                props.push(`name=${e.getAttribute("name")}`);
              if (e.getAttribute("placeholder"))
                props.push(`placeholder="${e.getAttribute("placeholder")}"`);
              if (e.getAttribute("aria-label"))
                props.push(`aria="${e.getAttribute("aria-label")}"`);

              const text = (e.innerText || e.getAttribute("value") || "")
                .trim()
                .substring(0, 30)
                .replace(/\n/g, " ");
        ...
        const simplifiedDom =
          dom.substring(0, 8000) || "No visible interactive elements.";
...
Respond strictly in JSON.`;
        ...
        try {
                ...
                .array(
                  z.object({
                    ...
                    selector_or_text: z
                      .string()
                      .describe(
                        "CSS selector, exact text, placeholder, or URL depending on action. Use empty string if not needed.",
                      ),
                    value: z
                      .string()
                      .describe(
                        "Value to fill if action is fill_css or fill_placeholder. Use empty string if not needed.",
                      ),
                  }),
                )
                .describe(
                  "A list of sequential Playwright actions needed to fully accomplish this test step.",
                ),
              reason: z
                .string()
          ...
          console.log(
            `[AI Explorer] AI Decision for Step ${i + 1} (Attempt ${attempt}):`,
            result.object,
          );
          ...
          if (actions.length === 0 || actions.every((a) => a.type === "none")) {
            if (attempt < maxAttempts) {
              console.log(
                `[AI Explorer] AI returned 'none'. Waiting 3s to let SPA load, then retrying...`,
              );
              await page.waitForTimeout(3000);
              continue;
            } else {
              generatedScriptLines.push(
                `  // AI could not determine action: ${reason}`,
              );
              break;
            }
          }
          ...
          for (const act of actions) {
            ...
            if (action === "click_css" && selector_or_text) {
              await page
                .locator(selector_or_text)
                .first()
                .click({ timeout: 5000 });
              await page.waitForTimeout(1000);
              await page.waitForLoadState("networkidle").catch(() => {});
              generatedScriptLines.push(
                `  await page.locator('${selector_or_text.replace(/'/g, "\\'")}').first().click();`,
              );
            } else if (action === "click_text" && selector_or_text) {
              await page
                .getByText(selector_or_text)
                .first()
                .click({ timeout: 5000 });
              await page.waitForTimeout(1000);
              await page.waitForLoadState("networkidle").catch(() => {});
              generatedScriptLines.push(
                `  await page.getByText('${selector_or_text.replace(/'/g, "\\'")}').first().click();`,
              );
            } else if (
              action === "fill_css" &&
              selector_or_text &&
              ...
              generatedScriptLines.push(
                `  await page.locator('${selector_or_text.replace(/'/g, "\\'")}').first().fill('${value.replace(/'/g, "\\'")}');`,
              );
            } else if (
              ...
              await page
                .getByPlaceholder(selector_or_text)
                .first()
                .fill(value, { timeout: 5000 });
              generatedScriptLines.push(
                `  await page.getByPlaceholder('${selector_or_text.replace(/'/g, "\\'")}').first().fill('${value.replace(/'/g, "\\'")}');`,
              );
              ...
              await page
                .goto(selector_or_text, { waitUntil: "networkidle" })
                .catch(() => {});
              generatedScriptLines.push(
                `  await page.goto('${selector_or_text}');`,
              );
            }
          }
          success = true;
          ...
          console.error(
            `[AI Explorer] Failed to execute step ${i + 1} (Attempt ${attempt}):`,
            e.message,
          );
          lastError = e.message;
          if (attempt < maxAttempts) {
            ...
            generatedScriptLines.push(
              `  // FAILED to execute step: ${safeErrorMessage}`,
            );
      ...
      if (!success) {
        generatedScriptLines.push(
          `  // ABORTING test generation because step ${i + 1} failed.`,
        );
        break;
      }
      ...
      if (success && step.expectedResult) {
        console.log(
          `[AI Explorer] Processing Phase 2 (Assertions) for Step ${i + 1}: ${step.expectedResult}`,
        );
        generatedScriptLines.push(
          `  // Verify Expected: ${step.expectedResult}`,
        );
        ...
        while (assertionAttempt < maxAttempts && !assertionSuccess) {
          ...
          const newDom = await page.evaluate(() => {
            const elements = document.querySelectorAll(
              'button, a, input, select, textarea, [role="button"], [role="link"], [role="textbox"], [role="menuitem"], h1, h2, h3, h4, p, span, div',
            );
            const visibleElements = Array.from(elements).filter((el) => {
              const e = el as HTMLElement;
              // Ignore very large wrapper divs to save tokens, focus on content
              if (e.tagName.toLowerCase() === "div" && e.children.length > 3)
                return false;
              return (
                e.offsetWidth > 0 &&
                e.offsetHeight > 0 &&
            ...
            return visibleElements
              .map((el) => {
                ...
                if (e.className && typeof e.className === "string")
                  props.push(`class=${e.className.split(" ")[0]}`); // Only take first class to save space
                if (e.getAttribute("placeholder"))
                  props.push(`placeholder="${e.getAttribute("placeholder")}"`);
          ...
          const simplifiedNewDom =
            newDom.substring(0, 8000) || "No visible text elements.";
...
Respond strictly in JSON.`;
          ...
          try {
                  ...
                  .array(
                    z.object({
                      ...
                      selector_or_text: z
                        .string()
                        .describe(
                          "CSS selector, exact text, or placeholder to assert. Use empty string if not needed.",
                        ),
                    }),
                  )
                  .describe(
                    "A list of Playwright assertions to verify the expected result.",
                  ),
            ...
            console.log(
              `[AI Explorer] AI Assertion Decision for Step ${i + 1} (Attempt ${assertionAttempt}):`,
              assertionResult.object,
            );

            if (
              assertions.length === 0 ||
              assertions.every((a) => a.type === "none")
            ) {
              if (assertionAttempt < maxAttempts) {
                console.log(
                  `[AI Explorer] AI returned 'none' for assertions. Waiting 3s...`,
                );
                await page.waitForTimeout(3000);
                continue;
              } else {
                generatedScriptLines.push(
                  `  // AI could not determine assertion: ${reason}`,
                );
                break;
              }
            }
            ...
            for (const ast of assertions) {
              ...
              if (ast.type === "assert_text" && ast.selector_or_text) {
                generatedScriptLines.push(
                  `  await expect(page.getByText('${ast.selector_or_text.replace(/'/g, "\\'")}').first()).toBeVisible();`,
                );
              } else if (ast.type === "assert_css" && ast.selector_or_text) {
                generatedScriptLines.push(
                  `  await expect(page.locator('${ast.selector_or_text.replace(/'/g, "\\'")}').first()).toBeVisible();`,
                );
              } else if (
                ast.type === "assert_placeholder" &&
                ast.selector_or_text
              ) {
                generatedScriptLines.push(
                  `  await expect(page.getByPlaceholder('${ast.selector_or_text.replace(/'/g, "\\'")}').first()).toBeVisible();`,
                );
              }
            }
            ...
            console.error(
              `[AI Explorer] Failed to generate assertion for step ${i + 1}:`,
              e.message,
            );
            ...
            if (assertionAttempt < maxAttempts) {
              ...
              const safeErrorMessage = e.message.replace(/\n/g, "\n  // ");
              generatedScriptLines.push(
                `  // FAILED to generate assertion: ${safeErrorMessage}`,
              );
              break;
            }
          }
    ...
    return NextResponse.json(
      { error: error.message || "Failed to explore and generate script" },
      { status: 500 },
    );
  } finally {
    if (browser) {
