...
const VALID_SEVERITY = ["NOT_SET", "BLOCKER", "CRITICAL", "MAJOR", "NORMAL", "MINOR", "TRIVIAL"];
...
export async function PATCH(req: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    const hasAccess = await requireProjectRole(project.code, (session.user as any).id, ["EDITOR", "ADMIN"]);
    if (!hasAccess && (session.user as any).role !== "ADMIN") {
      return NextResponse.json({ error: "Forbidden: You do not have permission to edit test cases in this project" }, { status: 403 });
    }
    ...
    if (fields?.priority && VALID_PRIORITY.includes(fields.priority)) data.priority = fields.priority;
    if (fields?.severity && VALID_SEVERITY.includes(fields.severity)) data.severity = fields.severity;
    if (fields?.automationStatus && VALID_AUTOMATION.includes(fields.automationStatus)) data.automationStatus = fields.automationStatus;
    ...
    return NextResponse.json({ error: "Failed to edit test cases" }, { status: 400 });
  }
}
