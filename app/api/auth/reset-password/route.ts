...
export async function GET(req: Request) {
  try {
    ...
    if (!rec) return NextResponse.json({ error: "Invalid or unknown reset link." }, { status: 404 });
    if (rec.usedAt) return NextResponse.json({ error: "This reset link has already been used." }, { status: 400 });
    if (new Date() > rec.expiresAt) return NextResponse.json({ error: "This reset link has expired." }, { status: 400 });
