...
export async function GET(req: NextRequest) {
  try {
    ...
    const disabledIds: string[] = disabledSetting
      ? JSON.parse(disabledSetting.value)
      : [];

    // DB overrides for system fields (created on first edit)
    const sysOverrides = Object.fromEntries(
      customFields
        .filter((f) => f.isSystem && f.id.startsWith("sys-"))
        .map((f) => [f.id, f]),
    );
    ...
    const SYS_DEFAULTS = [
      ...
      {
        id: "sys-automation-status-dep",
        name: "Automation status (deprecated)",
        type: "SELECT",
        isRequired: true,
        order: 9,
      },
      {
        id: "sys-automation-status",
  ...
  try {
    ...
    const field = await prisma.customField.create({
      data: {
        ...
        isSystem: false,
        projects:
          !isGlobalVal && Array.isArray(projectIds) && projectIds.length > 0
