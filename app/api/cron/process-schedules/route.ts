  ...
  try {
    ...
    for (const schedule of schedules) {
      try {
        ...
        if (isDue) {
          console.log(
            `[CRON] Triggering schedule: ${schedule.title} (ID: ${schedule.id})`,
          );
          ...
          if (!GITHUB_TOKEN || !GITHUB_OWNER || !GITHUB_REPO) {
            console.error(
              `[CRON] GitHub integration missing for project: ${project.code}`,
            );
            continue;
          }
          ...
          if (testCases.length === 0) {
            console.log(
              `[CRON] No automated test cases found for project: ${project.code}`,
            );
            continue;
          }
          ...
          if (!ghRes.ok) {
            console.error(
              `[CRON] Failed to trigger GitHub Action for schedule ${schedule.id}`,
              await ghRes.text(),
            );
          } else {
        ...
        console.error(
          `[CRON] Failed to parse cron or trigger for schedule ${schedule.id}:`,
          err,
        );
      }
    }
