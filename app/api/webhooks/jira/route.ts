...
export async function POST(req: Request) {
  try {
    ...
    if (payload.webhookEvent === "jira:issue_updated" && payload.issue) {
      ...
      const newRequirementText = payload.issue.fields?.description || payload.issue.fields?.summary || "";

      if (!newRequirementText) {
        return NextResponse.json({ message: "No requirement text found in Jira payload" });
      }
