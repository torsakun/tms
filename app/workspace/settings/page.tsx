...
import { Settings, Save, AlertCircle, Sparkles, CheckCircle2 } from "lucide-react";
    ...
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden transition-colors">
      <header className="px-8 py-6 border-b border-border flex justify-between items-center bg-surface shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-2xl font-bold text-text-main">Workspace Settings</h1>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
        ...
        <div className="max-w-3xl space-y-8">
          ...
          <section className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-border bg-background/50 flex items-center">
              ...
              <h2 className="text-lg font-bold text-text-main">AI Integrations</h2>
            </div>
            <div className="p-6 space-y-6">
              <p className="text-sm text-text-muted">
                Configure your API keys to enable AI-powered features like the Test Case Generator. Keys are stored securely in the database.
              </p>

              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">OpenAI API Key</label>
                <input 
                  type="password" 
                  value={openaiKey}
                ...
                />
                <p className="text-xs text-text-muted mt-1">Used for GPT-4o models.</p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">Google Gemini API Key</label>
                <input 
                  type="password" 
                  value={geminiKey}
                  ...
                  className="w-full px-4 py-2.5 bg-background border border-border text-text-main rounded-lg focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-[0_2px_10px_rgba(0,0,0,0.02)] placeholder:text-text-muted/50"
                />
                <p className="text-xs text-text-muted mt-1">Used for Gemini 1.5 Pro models.</p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">Anthropic Claude API Key</label>
                <input 
                  type="password" 
                  value={claudeKey}
                  ...
                  className="w-full px-4 py-2.5 bg-background border border-border text-text-main rounded-lg focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-[0_2px_10px_rgba(0,0,0,0.02)] placeholder:text-text-muted/50"
                />
                <p className="text-xs text-text-muted mt-1">Used for Claude 3.5 Sonnet models.</p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="p-6 space-y-6">
              <p className="text-sm text-text-muted">
                Connect your Jira workspace to allow the AI to automatically fetch user stories and acceptance criteria from Jira Tickets.
              </p>

              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">Jira Domain</label>
                <input 
                  type="text" 
                  value={jiraDomain}
              ...
              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">Jira Email</label>
                <input 
                  type="email" 
                  value={jiraEmail}
              ...
              <div>
                <label className="block text-sm font-semibold text-text-main mb-1">Jira API Token</label>
                <input 
                  type="password" 
                  value={jiraToken}
          ...
          <section className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-border bg-background/50 flex items-center">
              <svg className="w-5 h-5 text-emerald-500 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
              </svg>
              <h2 className="text-lg font-bold text-text-main">Data Backup & Export</h2>
            </div>
            <div className="p-6 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border border-border rounded-lg bg-background">
                <div>
                  <h3 className="text-sm font-bold text-text-main">Manual JSON Export</h3>
                  <p className="text-xs text-text-muted mt-1">
                    Download a complete JSON snapshot of your workspace, including all projects, test cases, suites, and test runs. 
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border border-blue-100 rounded-lg bg-blue-50/50">
                <div>
                  <h3 className="text-sm font-bold text-blue-900">Automated SQL Backups</h3>
                  <p className="text-xs text-blue-700 mt-1">
                    Automated full database dumps are running nightly via GitHub Actions.
                    Backups are securely stored as GitHub Artifacts and retained for 30 days.
                  </p>
                </div>
