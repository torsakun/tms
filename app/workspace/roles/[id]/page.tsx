...
const PERMISSION_BLOCKS = [
  ...
  {
    id: "test-plans", title: "Test plans",
...
function buildPermMap(permissions: string[]): Record<string, boolean> {
  ...
  if (permissions.includes("all")) {
    ALL_RULE_IDS.forEach(id => { perms[id] = true; });
  } else {
    ...
    fetch(`/api/workspace/roles/${roleId}`)
      ...
      .then(data => {
        if (data.success) {
          ...
          setSelectedPermissions(buildPermMap(Array.isArray(role.permissions) ? role.permissions : []));
        }
        setIsLoading(false);
      })
    ...
    <div className="w-full max-w-[900px] mx-auto px-6 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Edit role</h1>
        </div>
        {isSystem && (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200 uppercase tracking-wide">
            System
          </span>
        )}
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={isSystem}
                className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 disabled:bg-slate-50 disabled:text-slate-400 transition-all"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 resize-none transition-all"
              />
            </div>
                ...
                <div key={block.id}>
                  ...
                  <div className="divide-y divide-slate-50">
                    {block.rules.map((rule) => (
                      <label key={rule.id} className="flex items-start gap-3 px-6 py-2.5 transition-colors cursor-pointer hover:bg-indigo-50/30">
                        <input
                          type="checkbox"
