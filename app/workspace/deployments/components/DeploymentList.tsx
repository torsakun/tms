...
export default function DeploymentList({ initialDeployments }: { initialDeployments: any[] }) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "BUILDING": return <Loader2 className="text-primary animate-spin" size={18} />;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default: return "bg-surface-hover text-text-main border-border";
    }
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-surface-hover/50 border-b border-border">
              ...
              <th className="px-4 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">Trigger</th>
              <th className="px-4 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">Commit</th>
              <th className="px-4 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">Time</th>
              <th className="px-4 py-3 text-xs font-bold text-text-muted uppercase tracking-wider text-right">Logs</th>
            </tr>
          </thead>
              ...
              <tr key={dep.id} className="hover:bg-surface-hover/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col">
                    <span className="text-sm font-mono text-text-main">{dep.commitHash || "N/A"}</span>
                    <span className="text-xs text-text-muted truncate max-w-[200px]">{dep.commitMessage || "-"}</span>
                  </div>
                </td>
