...
const fetcher = (url: string) => fetch(url).then((res) => {
  if (!res.ok) {
    if (res.status === 403) throw new Error("This report is not public or has been disabled.");
...
export default function PublicReportPage({ params }: { params: Promise<{ runId: string }> }) {
  const { runId } = use(params);
  
  // Poll every 3 seconds for real-time updates
  const { data: run, error, isLoading } = useSWR(`/api/public/reports/${runId}`, fetcher, {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-[1000px] mb-4 flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium text-gray-500 uppercase tracking-wide">Live Report</span>
        </div>
