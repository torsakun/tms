...
export function ProjectSidebar({ projectCode }: { projectCode: string }) {
  ...
  const menuGroups = [
    {
      ...
      items: [
        {
          name: "Dashboards",
          href: `/projects/${projectCode}/dashboards`,
          icon: FileBarChart,
        },
      ],
    },
    {
      ...
      items: [
        {
          name: "Repository",
          href: `/projects/${projectCode}/repository`,
          icon: ListTodo,
        },
      ],
    },
    {
      ...
      items: [
        {
          name: "Test Plans",
          href: `/projects/${projectCode}/plans`,
          icon: ClipboardList,
        },
        {
          name: "Test Runs",
          href: `/projects/${projectCode}/runs`,
          icon: PlayCircle,
        },
      ],
    },
    {
      ...
      items: [
        {
          name: "Automation",
          href: `/projects/${projectCode}/automation`,
          icon: Bot,
        },
      ],
    },
    ...
    return (
      <li>
        <Link
          href={href}
          className={cn(
            "flex items-center gap-3 px-2.5 h-9 rounded-xl text-sm font-medium transition-all",
            !isActiveCheck && "hover:bg-surface-hover",
          )}
          style={isActiveCheck ? { background: "var(--primary-light)" } : {}}
        >
          <span
            className="shrink-0 w-6 h-6 rounded-lg flex items-center justify-center transition-all"
            style={{
              background: isActiveCheck ? s.iconBg : "var(--bg-surface-hover)",
            }}
          >
            <Icon
              size={13}
              style={{ color: s.iconColor, opacity: isActiveCheck ? 1 : 0.65 }}
            />
          </span>
          <span
            className="truncate font-semibold text-[13px]"
            style={{
              color: isActiveCheck ? "var(--text-main)" : "var(--text-muted)",
            }}
          >
            {name}
          </span>
  ...
  return (
    <aside
      className="w-[220px] flex flex-col h-full overflow-y-auto shrink-0"
      style={{
    ...
    >
      ...
      <div
        className="flex-1 flex flex-col gap-4 px-3 pb-4"
      ...
      >
        ...
        {(role === "ADMIN" || isSystemAdmin) && (
          ...
          >
            ...
            <NavItem
              ...
              icon={Settings}
              isActiveCheck={pathname === `/projects/${projectCode}/settings`}
            />
            <NavItem
              name="Integrations"
              href={`/projects/${projectCode}/settings/integrations`}
              icon={Settings2}
              isActiveCheck={pathname.startsWith(
                `/projects/${projectCode}/settings/integrations`,
              )}
            />
            <NavItem
              name="Audit Logs"
        ...
        <div className="pt-2 space-y-0.5">
          <NavItem
            ...
            icon={Share2}
            isActiveCheck={pathname.startsWith(
              `/projects/${projectCode}/shared-steps`,
            )}
          />
          <NavItem
            name="Environments"
            ...
            icon={Box}
            isActiveCheck={pathname.startsWith(
              `/projects/${projectCode}/environments`,
            )}
          />
        </div>
      </div>
