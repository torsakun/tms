...
export function InviteUserModal({ onClose }: InviteUserModalProps) {
  ...
  const inputCls = "w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 transition-all";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden border border-slate-100">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-bold text-slate-800">Invite new member</h2>
          </div>
        ...
        {/* Tabs */}
        <div className="px-6 border-b border-slate-100 flex gap-5 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {activeTab === "single" ? (
            <form id="invite-form" onSubmit={handleSubmit} className="space-y-4">
              {error && (
                <div className="p-3 text-sm text-rose-600 bg-rose-50 rounded-lg border border-rose-100">{error}</div>
              )}
              {success && (
              ...
              <div>
                ...
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="john.smith@company.com" className={inputCls} />
              </div>

              <div>
                <label className={labelCls}>Role <span className="text-rose-400">*</span></label>
                {isLoadingRoles ? (
            ...
            <div className="py-8 text-center">
              ...
              <p className="text-sm font-semibold text-slate-500">Bulk invite coming soon</p>
              <p className="text-xs text-slate-400 mt-1">Use the Single tab to invite members one at a time.</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100 shrink-0">
          ...
          <button form="invite-form" type="submit"
            ...
            {isSubmitting ? <><Loader2 size={14} className="animate-spin" /> Sending…</> : "Invite"}
          </button>
        </div>
      </div>
    </div>
