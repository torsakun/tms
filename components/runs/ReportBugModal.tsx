...
export function ReportBugModal({
  ...
  const draft = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/runs/${runId}/results/${result.id}/triage`,
        {
          method: "POST",
      ...
      if (res.ok) {
        ...
        setSummary(
          result.testCase?.title ? `[Bug] ${result.testCase.title}` : "",
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/runs/${runId}/results/${result.id}/report-bug`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {steps.length === 0 && (
                  <p className="text-xs text-text-muted italic">No steps</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="px-6 py-4 bg-surface-hover/60 border-t border-border flex justify-between items-center gap-3">
          <button
            onClick={draft}
