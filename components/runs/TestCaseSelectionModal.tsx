...
import { X, Search, ChevronRight, ChevronDown, CheckSquare, Square, StopCircle } from "lucide-react";
...
export function TestCaseSelectionModal({ isOpen, onClose, onSave, suites, cases, initialSelectedIds }: Props) {
  ...
  const { casesBySuite, allCaseIds, unassignedCases } = useMemo(() => {
    ...
    return { casesBySuite: map, allCaseIds: allIds, unassignedCases: unassigned };
  }, [cases]);
  ...
  const isAllSelected = selectedIds.size === allCaseIds.size && allCaseIds.size > 0;
  const isSomeSelected = selectedIds.size > 0 && selectedIds.size < allCaseIds.size;
  ...
  const toggleSuite = (suiteId: string, suiteCaseIds: string[]) => {
    ...
    const allSelected = suiteCaseIds.length > 0 && suiteCaseIds.every(id => newSelected.has(id));
  ...
  const toggleUnassigned = () => {
    ...
    const newSelected = new Set(selectedIds);
    const allSelected = ids.length > 0 && ids.every(id => newSelected.has(id));
    ids.forEach(id => {
  ...
  const renderSuiteTree = (suiteList: Suite[], level: number = 0) => {
    return suiteList.map(suite => {
      ...
      const selectedCount = suiteCaseIds.filter(id => selectedIds.has(id)).length;
      const totalCount = suiteCaseIds.length;
      const isSuiteSelected = selectedCount === totalCount && totalCount > 0;
      const isSuiteIndeterminate = selectedCount > 0 && selectedCount < totalCount;
      ...
      return (
        <div key={suite.id} className="select-none">
          <div 
            ...
            style={{ paddingLeft: `${ level * 1.5 + 0.75 }rem`, paddingRight: "1rem", paddingTop: "0.5rem", paddingBottom: "0.5rem" }}
            onClick={() => setActiveSuiteId(suite.id)}
          >
            <div className="w-5 h-5 flex items-center justify-center shrink-0 mr-1 opacity-50 hover:opacity-100" onClick={toggleExpand}>
            ...
            <button 
              ...
              onClick={(e) => { e.stopPropagation(); toggleSuite(suite.id, suiteCaseIds); }}
            >
              {isSuiteSelected ? (
            ...
            <span className="text-sm font-medium text-text-main truncate flex-1 leading-none">{suite.title}</span>
            <span className="text-xs text-text-muted font-mono shrink-0 ml-2">{selectedCount}/{totalCount}</span>
          </div>
      ...
      <div className="flex-1 bg-surface rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col overflow-hidden w-full max-w-6xl mx-auto border border-border">
        ...
        <header className="px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-3">
            <h1 className="text-xl font-bold text-text-main">Select test cases</h1>
            <span className="text-sm font-medium text-text-muted">{selectedIds.size} case{selectedIds.size !== 1 && 's'} selected</span>
          </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <aside className="w-[300px] border-r border-border/50 flex flex-col bg-surface overflow-y-auto shrink-0">
            <div className="py-2">
              ...
              >
                ...
                <span className="text-sm font-semibold text-text-main truncate flex-1">Select all</span>
                <span className="text-xs text-text-muted font-mono shrink-0">{selectedIds.size}/{allCaseIds.size}</span>
              </div>
              ...
              >
                ...
                <button 
                  type="button" 
                  className="mr-2 text-text-muted flex shrink-0" 
                  onClick={(e) => { e.stopPropagation(); toggleUnassigned(); }}
                >
                  {unassignedCases.length > 0 && unassignedCases.every(c => selectedIds.has(c.id)) ? (
                    <CheckSquare size={16} className="text-primary" />
                ...
                <span className="text-sm font-medium text-text-main truncate flex-1">Test cases without suite</span>
                <span className="text-xs text-text-muted font-mono shrink-0">
                  {unassignedCases.filter(c => selectedIds.has(c.id)).length}/{unassignedCases.length}
                </span>
              </div>
          ...
          <main className="flex-1 flex flex-col bg-surface overflow-hidden min-w-0">
            <header className="px-6 py-4 border-b border-border/50 shrink-0">
              <h2 className="text-lg font-bold text-text-main">{displayTitle}</h2>
            </header>
            ...
            <div className="px-6 py-2 border-b border-border/50 flex items-center bg-background shrink-0 select-none">
              ...
              <span className="text-sm font-semibold text-text-main">Select all</span>
              <span className="text-xs font-mono text-text-muted ml-2">{activePaneSelectedCount}/{activeSuiteSelectionIds.length}</span>
            </div>

            <div className="flex-1 overflow-y-auto p-4">
              {displayCases.length === 0 ? (
                <div className="flex items-center justify-center h-full text-text-muted text-sm">No cases in this suite.</div>
              ) : (
                <div className="space-y-1">
                  {displayCases.map(c => (
                    ...
                    >
                      <button type="button" className="mr-3 text-text-muted focus:outline-none shrink-0">
                      ...
                      <div className="flex-1 flex items-center min-w-0">
                        <span className="text-sm font-medium text-text-main truncate mr-3">{c.code || "TC-?"}</span>
                        <span className="text-sm text-text-main truncate">{c.title}</span>
                      </div>
                      <div className="shrink-0 ml-4 flex space-x-2">
                         <span className="px-2 py-0.5 bg-background border border-border text-text-muted rounded text-[10px] font-bold uppercase">{c.status || "Active"}</span>
                      </div>
                    </div>
                  ))}
                </div>
