...
export function TestCaseAutomationPanel({
  testCase,
  projectCode,
  onUpdate,
}: TestCaseAutomationPanelProps) {
  ...
  const [verificationLog, setVerificationLog] = useState<string | null>(
    testCase.lastVerificationLog || null,
  );
  const [isVerificationExpanded, setIsVerificationExpanded] = useState(
    !!testCase.lastVerificationLog,
  );

  const [isCreatingPR, setIsCreatingPR] = useState(false);
  const [githubPrUrl, setGithubPrUrl] = useState<string | null>(
    testCase.githubPrUrl || null,
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/ai/generate-script`,
        {
          method: "POST",
  ...
  const handleExplore = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/ai/explore`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || "Failed to explore and generate script");

      setScript(data.script);
      setSuccess(
        "Agent successfully explored the app and generated the script!",
      );
  ...
  const handleAutoFix = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/ai/fix-script`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            script,
            errorLog: verificationLog,
            domContext: autoCapturedDom || domContext,
          }),
        },
      );
      ...
      if (data.script.trim() === script.trim()) {
        setSuccess(
          "AI could not find a fix. Try pasting the page HTML into the 'DOM Snippet' field first!",
        );
      } else {
        setScript(data.script);
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.passed) {
        setSuccess(
          "Script verification passed! You can now save it to the case.",
        );
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/script`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ script }),
  ...
  const handleCreatePR = async () => {
    if (
      !confirm(
        "This will commit the script to your connected GitHub repository and open a Pull Request. Proceed?",
      )
    )
      return;
    ...
    try {
      ...
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/github/pr`,
        {
          method: "POST",
        },
      );
        ...
        <div className="mb-6 border border-border rounded-lg overflow-hidden bg-[#0d1117]">
          ...
          >
            ...
            <div className="flex items-center space-x-3">
              ...
              {(verificationLog.includes("failed") ||
                verificationLog.includes("Error") ||
                verificationLog.includes("timeout")) && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleAutoFix();
        ...
        {isExpanded && (
          <div className="p-4 border-t border-border">
            <p className="text-xs text-text-muted mb-2">
              Paste a snippet of HTML or Page Object Model to help the AI map
              exact locators instead of guessing.
            </p>
            <textarea
              value={domContext}
      ...
      {!script && !isGenerating && !isExploring && (
        <div className="flex flex-col items-center justify-center py-8 border-2 border-dashed border-border rounded-lg bg-background/50">
          ...
          <p className="text-xs text-text-muted mt-4 text-center max-w-md">
            <b>Agentic Mode:</b> The AI will autonomously open a browser,
            navigate, and build the script. (Takes 30-60s)
          </p>
        </div>
      )}
      ...
      {isGenerating && (
        <div className="flex flex-col items-center justify-center py-12 border border-border rounded-lg bg-background/50">
          ...
          <p className="text-xs text-text-muted mt-1">
            Analyzing steps and guessing locators
          </p>
        </div>
      )}
      ...
      {isExploring && (
        <div className="flex flex-col items-center justify-center py-12 border border-border rounded-lg bg-background/50">
          ...
          <p className="text-sm font-bold text-text-main">
            AI is exploring the app...
          </p>
          <p className="text-xs text-text-muted mt-1 text-center max-w-xs">
            Opening headless browser, analyzing DOM, clicking elements, and
            writing code. Please wait up to 60 seconds.
          </p>
        </div>
      )}
          ...
          {isFixing && (
            <div className="absolute inset-0 bg-[#0d1117]/80 backdrop-blur-[2px] flex flex-col items-center justify-center rounded-lg z-10 border border-slate-800">
              ...
              <p className="text-sm font-bold text-white">
                AI is fixing the script...
              </p>
              <p className="text-xs text-text-muted mt-1">
                Analyzing Playwright logs and correcting errors
              </p>
            </div>
          )}
