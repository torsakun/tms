...
export function RepositoryHeader({
  ...
  if (selectedCases.size > 0) {
    return (
      <div
        className="flex-none px-6 py-3.5 border-b bg-surface"
        style={{
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold text-text-main">Repository</span>
          ...
          <span className="flex items-center gap-1.5 text-sm font-semibold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">
            {selectedCases.size} selected · {selectedSuitesCount} suite
            {selectedSuitesCount !== 1 ? "s" : ""}
            <button
