...
import { X, Sparkles, Ticket, AlertTriangle, CheckCircle2, Loader2, Database, Folder } from "lucide-react";
...
export function BulkJiraImpactModal({ isOpen, onClose, projectCode, suites, allCases, onSuccess }: BulkJiraImpactModalProps) {
  ...
  const [targetMode, setTargetMode] = useState<"SUITE" | "JIRA_LINKED">("SUITE");
  ...
  const handleAnalyze = async () => {
    ...
    if (targetMode === "SUITE") {
      ...
      targetCaseIds = allCases.filter(c => c.suiteId === targetSuiteId).map(c => c.id);
    } else {
      // Find cases linked to this Jira ticket
      targetCaseIds = allCases.filter(c => c.jiraId === jiraTicketId).map(c => c.id);
    }

    if (targetCaseIds.length === 0) {
      setError(`No test cases found for the selected target (${targetMode === 'SUITE' ? 'Suite' : 'Jira ID'}).`);
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to analyze impact in bulk");
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      for (const res of impactResults) {
        if (selectedCaseIds.has(res.caseId) && res.success && res.result.needsUpdate) {
          const originalCase = allCases.find(c => c.id === res.caseId);
          updates.push({
            id: res.caseId,
            title: res.result.suggestedUpdates?.title || originalCase.title,
            description: res.result.suggestedUpdates?.description || originalCase.description,
            preconditions: res.result.suggestedUpdates?.preconditions || originalCase.preconditions,
            steps: res.result.suggestedUpdates?.steps?.map((s: any, idx: number) => ({
              action: s.action,
      ...
      const res = await fetch(`/api/projects/${projectCode}/cases/bulk-update`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ updates, requirementText: newRequirementText })
      });
  ...
  const casesNeedingUpdate = impactResults.filter(r => r.success && r.result?.needsUpdate);
  const casesUnchanged = impactResults.filter(r => r.success && !r.result?.needsUpdate);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface w-[1000px] max-h-[95vh] rounded-xl shadow-2xl overflow-hidden border border-border flex flex-col animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-indigo-50 shrink-0">
          ...
          <button onClick={onClose} className="text-indigo-600 hover:text-indigo-900 transition-colors">
            <X size={20} />
          </button>
        </div>
            ...
            <div className="space-y-6 flex-1 flex flex-col max-w-3xl mx-auto w-full">
              ...
              <div className="bg-surface p-6 rounded-xl border border-border">
                ...
                {newRequirementText !== null && (
                  <div className="mt-4">
                    <label className="block text-xs font-semibold text-text-muted mb-1">Requirement Text (Editable)</label>
                    <textarea 
              ...
              <div className="bg-surface p-6 rounded-xl border border-border">
                ...
                <div className="flex space-x-4 mb-4">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="targetMode" checked={targetMode === "SUITE"} onChange={() => setTargetMode("SUITE")} className="text-indigo-600 focus:ring-indigo-500" />
                    <span className="text-sm font-medium text-text-main">By Test Suite</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="targetMode" checked={targetMode === "JIRA_LINKED"} onChange={() => setTargetMode("JIRA_LINKED")} className="text-indigo-600 focus:ring-indigo-500" />
                    <span className="text-sm font-medium text-text-main">By Linked Jira ID ({jiraTicketId || 'Ticket'})</span>
                  </label>
                </div>
                ...
                {targetMode === "SUITE" && (
                  ...
                  >
                    ...
                    {suites.map(s => (
                      <option key={s.id} value={s.id}>{s.title} ({allCases.filter(c => c.suiteId === s.id).length} cases)</option>
                    ))}
                  </select>
                )}
                {targetMode === "JIRA_LINKED" && (
                  <div className="p-3 bg-blue-50 text-blue-800 rounded-md text-sm">
                    Will analyze all test cases in this project that have their `jiraId` set to <strong>{jiraTicketId || 'the provided ticket'}</strong>.
                    ({allCases.filter(c => c.jiraId === jiraTicketId).length} cases found)
                  </div>
                )}
              </div>
              ...
              <div className="bg-surface p-6 rounded-xl border border-border">
                ...
                <select 
                  ...
                  className="w-full px-4 py-2.5 bg-background border border-border text-text-main rounded-lg focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                >
                  <option value="openai">OpenAI (GPT-4o) - Recommended for logic</option>
                  <option value="gemini">Google (Gemini 1.5 Pro) - High Context</option>
                  <option value="claude">Anthropic (Claude 3.5 Sonnet) - Best at nuance</option>
                </select>
              </div>
          ...
          {step === "LOADING" && (
            <div className="flex flex-col items-center justify-center py-32 flex-1">
              <div className="relative w-24 h-24 mb-8">
                ...
                <div className="absolute inset-2 bg-indigo-500/40 rounded-full animate-ping" style={{ animationDelay: '0.2s' }}></div>
              ...
              <h3 className="text-2xl font-bold text-text-main mb-2">Analyzing Test Cases in Bulk...</h3>
              <p className="text-text-muted">Comparing the new requirement against each test case.</p>
            </div>
          )}
          ...
          {step === "REVIEW" && (
            <div className="space-y-6">
              <div className="flex items-center justify-between bg-surface p-4 rounded-lg border border-border">
                <div>
                  <h4 className="font-bold text-text-main">Impact Analysis Results</h4>
                  <p className="text-sm text-text-muted">Analyzed {impactResults.length} cases. Found {casesNeedingUpdate.length} cases that need updates.</p>
                </div>
                <div className="flex space-x-4 text-sm font-medium">
                  <span className="text-amber-600 bg-amber-50 px-3 py-1 rounded-full">{casesNeedingUpdate.length} Updates</span>
                  <span className="text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{casesUnchanged.length} Unchanged</span>
                  {casesFailed.length > 0 && <span className="text-red-600 bg-red-50 px-3 py-1 rounded-full">{casesFailed.length} Errors</span>}
                </div>
              </div>

              {casesNeedingUpdate.length > 0 && (
                <div className="space-y-4">
                  <h5 className="font-bold text-text-main text-lg">Test Cases to Update</h5>
              ...
              {casesFailed.length > 0 && (
                <div className="mt-8">
                  <h5 className="font-bold text-red-600 text-sm mb-3">Failed Analysis ({casesFailed.length} cases)</h5>
                  <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                    <ul className="list-disc pl-5 space-y-1">
                      {casesFailed.map(res => (
                        <li key={res.caseId} className="text-sm text-red-700">
                          <strong>{res.originalTitle}</strong> - <span className="text-xs">{res.error}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
