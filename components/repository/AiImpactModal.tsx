...
export function AiImpactModal({
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}/ai/impact`,
        {
          method: "POST",
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const newSteps =
        impactResult.suggestedUpdates.steps?.map((s: any, idx: number) => ({
          action: s.action,
      ...
      const res = await fetch(
        `/api/projects/${projectCode}/cases/${testCase.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            description:
              impactResult.suggestedUpdates.description || testCase.description,
            preconditions:
              impactResult.suggestedUpdates.preconditions ||
          ...
          {step === "INPUT" && (
            <div className="space-y-6 flex-1 flex flex-col">
              <div className="bg-blue-50 border border-blue-200 p-4 rounded-md text-sm text-blue-800">
                ...
                The requirement for this test case has changed in Jira. Paste
                the new requirement below to analyze the impact.
              </div>
          ...
          {step === "LOADING" && (
            <div className="flex flex-col items-center justify-center py-20 flex-1">
              ...
              <h3 className="text-xl font-bold text-text-main mb-2">
                Analyzing Requirement Diff...
              </h3>
              <p className="text-text-muted">
                Determining which test steps need updates.
              </p>
            </div>
          )}
                  ...
                  {impactResult.suggestedUpdates.steps && (
                    <div>
                      ...
                      <div className="bg-background border border-border rounded-lg overflow-hidden">
                        {impactResult.suggestedUpdates.steps.map(
                          (step: any, idx: number) => (
                            <div
                              key={idx}
