...
if [ ! -z "$ID" ] && [ "$ID" != "null" ]; then
  ...
  if [ $BUILD_STATUS -eq 0 ]; then
    ...
    # Wait for the application to be up and running before sending the final status
    sleep 20
    
    # Retry curl up to 10 times if it fails
    for i in {1..10}; do
      HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST -H "Authorization: Bearer $SECRET" -H "Content-Type: application/json" -d "{\"status\":\"SUCCESS\",\"logs\":$LOGS}" $API_URL/api/deployments/$ID/update)
